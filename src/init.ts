...
export const appInit = async (apiConfig?: ApiConfig, hostConfig?: HostConfig): Promise<InitConfig> => {
  ...
  // Setup axios base URL for file operations
  ...
  // Init authentication (loads tokens from localStorage if available)
  await store.dispatch('auth/initAuth')

  // With WebSocket-first approach, database loading and auth check
  // will happen after WebSocket connection is established.
  // Mark as connected if we have a valid URL.
  const apiConnected = apiConfig.apiUrl !== '' && apiConfig.socketUrl !== ''
  const apiAuthenticated = true // Will be determined by WebSocket connection
